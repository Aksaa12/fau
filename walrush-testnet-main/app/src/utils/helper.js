...
export class Helper {
  ...
  static delay(duration, taskId, taskName, extraInfo) {
    return new Promise((resolve) => {
      ...
      const logDelay = (time) => {
        const message = `Delaying for ${this.msToTime(time)}`;
        if (taskId !== undefined) {
  ...
  static determineType(input) {
    if (this.isMnemonic(input)) {
      ...
      return this.isPrivateKey(input) ? "Private Key" : "Unknown";
    }
  }
  ...
  static showLogo() {
    ...
    centerLog(
      `${ConsoleColors.GREEN}WALRUS TESTNET BOT ${ConsoleColors.BLACK}| ${ConsoleColors.GREEN}@autosultan_group${ConsoleColors.RESET} ${ConsoleColors.BLACK}| ${ConsoleColors.GREEN}@airdropasc `
    );
