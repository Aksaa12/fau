hapus fungsi kode yang tidak berkaitan dengan staking wal ke node walrus : import { decodeSuiPrivateKey } from "@mysten/sui/cryptography";
import {
...
export default class Core {
  ...
  try {
    // Merge coins if there are multiple coins of the same type
    await this.mergeCoin();
    await Helper.delay(1000, this.acc, "Trying to stake WAL to operator", this);
    
    // Get the WAL coins owned by the account
    const coins = await this.client.getCoins({
      owner: this.address,
      coinType: COINENUM.WAL,
    });

    // Ensure that there is at least one coin to stake
    if (!coins.data || coins.data.length === 0) {
      await Helper.delay(1000, this.acc, "No WAL coins available to stake", this);
      return;
    }

    // Get the first WAL coin
    const coin = coins.data[0];
    const amountToStake = 1 * MIST_PER_SUI; // Staking 1 WAL, convert to mist

    // Get the pool object and operator object
    ...
    // Split the coin to stake the specified amount
    const coinToStake = await transaction.splitCoins(
      transaction.object(coin.coinObjectId),
      [amountToStake]
    );

    // Call the staking function with the pool and operator object
    ...
    // Transfer the staked coin to the user's address
